...
namespace ltrModulesNet
{
    public class ltr25api
    {
        ...
        static extern _LTRNative.LTRERROR LTR25_Init(ref TLTR25 hnd);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_Close(ref TLTR25 hnd);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_Open(ref TLTR25 hnd, uint saddr, ushort sport, string csn, int slot_num);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_IsOpened(ref TLTR25 hnd);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_GetConfig(ref TLTR25 hnd);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_SetADC(ref TLTR25 hnd);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_Start(ref TLTR25 hnd);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_Stop(ref TLTR25 hnd);
        [DllImport("ltr25api.dll")]
        static extern int LTR25_Recv(ref TLTR25 hnd, uint[] data, uint[] tmark, uint size, uint timeout);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_ProcessData(ref TLTR25 hnd, uint[] src,
                                                            double[] dest, ref int size, ProcFlags flags,
                                                            ChStatus[] ch_status);

        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_SearchFirstFrame(ref TLTR25 hnd, uint[] data, uint size,
                                                                 out uint index);
        [DllImport("ltr25api.dll")]
        ...
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_FlashRead(ref TLTR25 hnd, uint addr, byte[] data, uint size);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_FlashWrite(ref TLTR25 hnd, uint addr, byte[] data, uint size);
        [DllImport("ltr25api.dll")]
        static extern _LTRNative.LTRERROR LTR25_FlashErase(ref TLTR25 hnd, uint addr, uint size);
        ...
        {
            return LTR25_Open(ref hnd, saddr, sport, csn, slot_num);
        }
        ...
        {
            return LTR25_Close(ref hnd);
        }

        public _LTRNative.LTRERROR IsOpened()
        {
            return LTR25_IsOpened(ref hnd);
        }

        public _LTRNative.LTRERROR GetConfig() 
        {
            return LTR25_GetConfig(ref hnd);
        }

        public _LTRNative.LTRERROR SetADC() 
        {
            return LTR25_SetADC(ref hnd);
        }

        public _LTRNative.LTRERROR Start() 
        {
            return LTR25_Start(ref hnd);
        }

        public _LTRNative.LTRERROR Stop() 
        {
            return LTR25_Stop(ref hnd);
        }

        public int Recv(uint[] data, uint[] tmark, uint size, uint timeout)
        {
            return LTR25_Recv(ref hnd, data, tmark, size, timeout);
        }

        public int Recv(uint[] data, uint size, uint timeout)
        {
            return LTR25_Recv(ref hnd, data, null, size, timeout);
        }

        public _LTRNative.LTRERROR ProcessData(uint[] src, double[] dest, ref int size, ProcFlags flags,
                                                    ChStatus[] ch_status)
        {
            return LTR25_ProcessData(ref hnd, src, dest, ref size, flags, ch_status);
        }

        public _LTRNative.LTRERROR ProcessData(uint[] src, double[] dest, ref int size, ProcFlags flags)
        {
            return LTR25_ProcessData(ref hnd, src, dest, ref size, flags, null);
        }

        public _LTRNative.LTRERROR SearchFirstFrame(uint[] data, uint size, out uint index) 
        {
            return LTR25_SearchFirstFrame(ref hnd, data, size, out index);
        }
